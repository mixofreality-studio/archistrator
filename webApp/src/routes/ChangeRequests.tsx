    ...
    >
      ...
      >
        ...
        <Chip
          label="0 in flight"
          size="small"
          sx={{ bgcolor: t.awaitingBg, color: t.awaitingFg, fontFamily: t.mono, fontWeight: 700 }}
        />
      </Box>
      ...
      <Box sx={{ flexGrow: 1, overflowY: 'auto', px: { xs: 2, md: 4 }, py: 3 }}>
        <Box sx={{ maxWidth: 1080 }}>
          ...
          >
            <Typography
              sx={{ fontFamily: t.mono, fontSize: 11, color: t.chatArchitectFg, lineHeight: 1.55 }}
            >
              ↩ A change request on a delivered system does NOT edit the original (now-closed)
              project — it spawns a <b>subproject</b>: a fresh,
              <b> scaled</b> mini System-Design → Project-Design → Construction → Redeploy cycle,
              forward-only off <b>main</b> under a <b>cr-NN</b> label.
              <b> Triage scales the System-Design depth</b>: a bug / scope change skips System
              Design; a new use case runs the <b>design-validation gate</b>.
            </Typography>
          </Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
            ...
            <Chip
              label="0"
              size="small"
              sx={{ height: 18, fontSize: 9.5, bgcolor: t.paperAlt, color: t.muted }}
            />
            <Box sx={{ flexGrow: 1 }} />
            <Button
              color="primary"
          ...
          {intakeOpen ? (
            <Paper sx={{ p: 2, mb: 2 }}>
              <TextField
                ...
                size="small"
                slotProps={{
                  htmlInput: { 'data-testid': UI_IDENTIFIERS.ChangeRequests.INTAKE_TITLE },
                }}
                sx={{ mb: 1.5 }}
              />
              <TextField
                ...
                size="small"
                slotProps={{
                  htmlInput: { 'data-testid': UI_IDENTIFIERS.ChangeRequests.INTAKE_BODY },
                }}
                sx={{ mb: 1.5 }}
              />
              <Box sx={{ display: 'flex', gap: 1 }}>
                <Button
                  disabled
                  color="primary"
                  data-testid={UI_IDENTIFIERS.ChangeRequests.INTAKE_SUBMIT}
                  size="small"
                  sx={{ fontSize: 11 }}
                  variant="contained"
                >
                ...
                <Button
                  color="inherit"
                  data-testid={UI_IDENTIFIERS.ChangeRequests.INTAKE_CANCEL}
                  size="small"
                  sx={{ fontSize: 11, color: t.ink, borderColor: t.line }}
                  variant="outlined"
                  onClick={() => {
                    setIntakeOpen(false);
                  }}
              ...
              <Typography sx={{ fontFamily: t.mono, fontSize: 9.5, color: t.muted, mt: 1 }}>
                The change-request intake backend is not yet provisioned — submit is disabled until
                the change-request read/write lands.
              </Typography>
            </Paper>
          ) : null}
          ...
          >
            ...
            <Box sx={{ maxWidth: 520, mx: 'auto' }}>
              ...
              >
                A change request becomes a scaled subproject after triage. The change-request
                registry is not carried by the project read projection yet, so this surface is empty
                — open a subproject from here once the change-request read lands.
              </Typography>
            </Box>
          </Paper>
        </Box>
