...
export function InterventionsTab({
  ...
  /** The project ID — passed to PolicyPanel to wire the review policy mutation. */
  projectId: string;
}): ReactNode {
  const t = useTokens();
  ...
  return (
    <Box
      data-testid={UI_IDENTIFIERS.Construction.INTERVENTIONS}
    ...
    >
      ...
      <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1.25 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
          <Typography
            sx={{ fontFamily: t.mono, fontSize: 11, letterSpacing: '0.08em', color: t.muted }}
          >
        ...
        {pauseOpen ? (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            <Typography sx={{ color: t.muted, fontSize: 12 }}>
              Signals operatorPauseRequested — cancels in-flight pipelines and records the project
              operator-paused. A reason is required.
            </Typography>
            <TextField
              fullWidth
