...
void test('the segments keep their Table A-1 proportions whatever the effort', () => {
  const nodes = tree(NONE_CRITICAL);
  const long = laneSpineFor(node(nodes, 'C-m1'), undefined).segments.map((s) => s.fraction);
  const short = laneSpineFor(node(nodes, 'C-e1'), undefined).segments.map((s) => s.fraction);
