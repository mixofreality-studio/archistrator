      ...
      <Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1, flexWrap: 'wrap' }}>
          ...
          <Typography sx={{ fontFamily: t.display, fontWeight: 800, fontSize: 18, color: t.ink }}>
            What you’re billed for hosting
          </Typography>
          <Tooltip title="The customer always sees run-rate + projected monthly + what-if. Pure read; queryCostProjection mutates no state. The aggregated hosting line is charged on the Billing surface.">
            <Typography sx={{ fontFamily: t.mono, fontSize: 10, color: t.muted }}>
              queryCostProjection · no state mutation
            </Typography>
          </Tooltip>
        </Box>
        ...
        >
          <BigMetric
            emphasize
            hint="extrapolated at current observed load"
            label="Run-rate (now)"
            t={t}
            unit="/ day"
            value={formatMoney(view.currentRunRate)}
          />
          <BigMetric
            emphasize
        ...
        <Paper sx={{ p: 2 }}>
          ...
          <Typography sx={{ fontFamily: t.body, fontSize: 11.5, color: t.muted, mb: 1.5 }}>
            Projected monthly cost as the autoscaler settles at each replica level — “if I scale up,
            what do I pay?”
          </Typography>
      ...
      <Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1, flexWrap: 'wrap' }}>
          <Chip
            label="OPERATOR VIEW"
            size="small"
            sx={{ height: 19, fontSize: 8.5, color: t.muted }}
            variant="outlined"
          />
          <Typography sx={{ fontFamily: t.display, fontWeight: 800, fontSize: 18, color: t.ink }}>
            Autoscaler — the third actor
          </Typography>
          <Chip
            ...
            sx={{
              height: 19,
              fontSize: 9,
              bgcolor: view.autoscaler.mode === 'Manual' ? t.awaitingBg : t.committedBg,
              color: view.autoscaler.mode === 'Manual' ? t.awaitingFg : t.committedFg,
            }}
          />
          <Box sx={{ flexGrow: 1 }} />
          {/*
        ...
        <Paper sx={{ p: 0, overflow: 'hidden' }}>
          ...
          >
            ...
            <Typography sx={{ fontFamily: t.mono, fontSize: 9.5, color: t.muted }}>
              · proposeDesiredState (with why)
            </Typography>
          </Box>
          {view.autoscaler.decisions.length === 0 ? (
            <Box sx={{ px: 2, py: 2, textAlign: 'center' }}>
              <Typography sx={{ fontFamily: t.mono, fontSize: 11, color: t.muted }}>
                No autoscaler decisions observed yet.
              </Typography>
            </Box>
          ) : (
            view.autoscaler.decisions.map((d, i) => (
              <DecisionRow
                d={d}
                key={`${d.at}-${String(i)}`}
                last={i === view.autoscaler.decisions.length - 1}
                t={t}
              />
            ))
          )}
          <Box sx={{ px: 2, py: 1, bgcolor: t.paperAlt, borderTop: `1.5px solid ${t.line}` }}>
            <Typography
              sx={{ fontFamily: t.body, fontSize: 10.5, color: t.muted, lineHeight: 1.45 }}
            >
              Resume from idle-pause is <b>traffic-driven</b> (Envoy + scale-from-zero), not an
              operator button. The engine DECIDES; the Manager EXECUTES by publishing desired state;
              the GitOps runtime converges.
            </Typography>
          </Box>
        </Paper>
      </Box>
...
function BigMetric({
  ...
  return (
    <Paper
      sx={{
        p: 2,
        borderLeft: emphasize === true ? `5px solid ${t.accent}` : undefined,
        bgcolor: emphasize === true ? t.paperAlt : undefined,
      }}
    >
      <Typography
        sx={{
      ...
      <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 0.75 }}>
        ...
        {unit !== undefined && (
          <Typography sx={{ fontFamily: t.mono, fontSize: 12, color: t.muted }}>{unit}</Typography>
        )}
      </Box>
      <Typography
        sx={{ fontFamily: t.body, fontSize: 11, color: t.muted, mt: 0.25, lineHeight: 1.4 }}
      >
        {hint}
      </Typography>
    </Paper>
...
function WhatIfCurve({ t, points }: { t: Tokens; points: WhatIfPoint[] }): ReactNode {
  ...
  const line = points
    .map(
      (p, i) =>
        `${i === 0 ? 'M' : 'L'}${sx(p.replicas).toFixed(1)},${sy(p.projectedMonthlyCost.minorUnits).toFixed(1)}`
    )
    .join(' ');
    ...
    <Box>
          ...
          return (
            ...
            >
              <circle
                cx={cx}
                cy={cy}
                fill={isActive ? t.accent : t.paper}
                r={isActive ? 6 : 4}
                stroke={t.accent}
      ...
      {active !== undefined && (
        ...
        >
          ...
          <Typography sx={{ fontFamily: t.display, fontWeight: 800, fontSize: 16, color: t.ink }}>
            {formatMoney(active.projectedMonthlyCost)}/mo
          </Typography>
        </Box>
      )}
    </Box>
  ...
  return (
    <Box
      sx={{
        ...
        borderLeft: `4px solid ${d.published ? t.accent : 'transparent'}`,
      }}
    >
      <Typography
        sx={{ fontFamily: t.mono, fontSize: 10.5, color: t.muted, minWidth: 96, pt: 0.15 }}
      >
        {formatEventTime(d.at)}
      </Typography>
      <Box sx={{ minWidth: 96 }}>
        <Box
          sx={{
        ...
        >
          ...
          <Typography sx={{ fontFamily: t.mono, fontWeight: 700, fontSize: 9.5, color: t.ink }}>
            {d.action}
          </Typography>
        </Box>
      </Box>
      <Typography
        sx={{ fontFamily: t.body, fontSize: 12, color: t.ink, lineHeight: 1.4, flexGrow: 1 }}
      >
        {d.reason}
        {!d.published && (
          <Box component="span" sx={{ fontFamily: t.mono, fontSize: 9, color: t.muted }}>
            {' '}
            · no republish
          </Box>
        )}
