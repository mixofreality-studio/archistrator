...
| `useProjectAskQuestions` (`useProjectDesignMutations`) | `projectDesignAskQuestions` | `ActivityExperienceContainer` (M0) |
...
1. ~~**`SdpReviewView`'s option rows enrol no comment anchor.**~~ **FIXED** in the final fix wave (`2c5cddfb`, I4) — see "Fixed in the final fix wave" below. It was: `armOption` armed `sdpOptionAnchor(o.solutionKind)` and nothing ever called `useRegisterAnchor` for it, so an SDP option comment could only ever be UNPLACED, on the live M0 gate as well as in history — in the one surface whose whole job is choosing between those options.
...
## Fixed in the final fix wave (`2c5cddfb`)

The whole-branch review found one Critical and three cheap Important items. All four landed in one commit, with the earmark file updated in the same wave so this file and the final review agree.

| # | What it was | Where the fix is |
|---|---|---|
| **C1** | `resolveSubmitVerb` answered a staged QUESTION with an `ask` verb BEFORE it looked at whether the rail had a question op. ~30 of the 32 gates have none (`ask: {kind:'none'}`, R2/GAP-6) and the container's ask handler returns early for them, so one staged question replaced **Approve and Send back** with a button that dispatched nothing. | `submitVerb.ts` gains `allowAsk` (default true — the design rails and the MCP widget are unchanged) and a `notice` line saying which questions will NOT be sent; `CommentMargin` gains `allowQuestions`, which removes the composer's Question toggle entirely on those rails, so the question cannot be staged in the first place. Both are driven from `verbs.ask.kind !== 'none'`. |
| **I3** | The table recorded the M0 ask as "no op". | Wrong diagnosis: `projectDesignAskQuestions` has always existed (`src/api/ops.gen.ts:168`). Only the HOOK was missing. `useProjectAskQuestions` is the Phase-2 twin of `useAskQuestions`, and `activityVerbs`'s new `projectAsk` target dispatches it — spec §6's "comments **and questions** allowed" at M0 is now true. |
| **I2** | The M0 approve sent `feedback.notes` alone and then `reset()` cleared the anchored comments: everything a reviewer pinned to an option or an activity row was silently dropped. | `SubmitSDPDecision`'s body carries one feedback field and no `comments` array, so `foldCommentsIntoNotes` (`components/comments/reviewBatch.ts`) folds them into it — free-form notes first, then one `<anchor path> — <text>` line each. |
| **I4** | Smaller thing 1 above. | An extracted `OptionRow` in `SdpReviewView` calls `useRegisterAnchor(sdpOptionAnchor(...))`, exactly as `ContractSignatureList`'s `OpRow` does. The ref sits on the row's FIRST CELL, because the row itself is `display: contents` and an element with no box measures as offset 0 — which would have placed every option card at the top of the margin, a worse lie than leaving it unplaced. |

`project-design-m0-history.json` moved its unplaceable CONTROL comment to a new third entry (`m0r1c3`, anchored to `$.options[kind=hybridSolution]` — an option The Method never assembles, so no row can ever enrol it). The two real anchors are now both asserted PLACED, which is what proves the enrolment rather than assuming it.

## Found in the same wave, NOT fixed (stage 6)

- **M5 — the `ConstructionActivityView.type` doc text is out of date.** It reads "service, frontend, testing, deployment, documentation, uiDesign or integration" and omits `requirements`, `architecture` and `projectDesign`, which the unified activity list has carried since stage 1. The text is authored in the typed contract (`.aiarch/state/project.json:18008`) and regenerated into the OAS (`webApp/src/contracts/schema.ts:838`) and the MCP tool descriptions (`server/internal/client/mcp/construction/construction_tools.gen.go:439`). Fix the source doc and regen; nothing reads it at runtime, so it is documentation drift, not a defect.
- **M6 — three dead paths on the `LEGACY_COMPONENTS_HOOKS_FILES` allowlist.** `webApp/eslint.platform.config.js:79` still lists `src/components/DesignHealthView.tsx`, `src/components/OperationalConceptsView.tsx` and `src/components/construction/PolicyPanel.tsx`; none of the three exists any more (checked with `ls` at this commit). Pre-existing — the three files were already absent at the branch base `5e09b0b6`, so the entries were dead before stage 5 began (the list only shrinks) — but a waiver for a file that is gone is a waiver nobody can retire. Prune them in stage 6; of the nine listed paths, six exist.
- **M7 — `PHASE_META.systemDesign.subtitle` uses `\u2192` escapes** (`webApp/src/contracts/adapters.ts:86`) where its two siblings carry plain prose. It renders correctly (`→`); it is only inconsistent with every other string in the file, and a reader grepping for the arrow will not find it.
