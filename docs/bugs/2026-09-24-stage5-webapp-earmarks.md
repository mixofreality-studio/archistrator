...
- **M6 — three dead paths on the `LEGACY_COMPONENTS_HOOKS_FILES` allowlist.** `webApp/eslint.platform.config.js:79` still lists `src/components/DesignHealthView.tsx`, `src/components/OperationalConceptsView.tsx` and `src/components/construction/PolicyPanel.tsx`; none of the three exists any more (checked with `ls` at this commit). Pre-existing — stage 5 deleted the files, and the list only shrinks — but a waiver for a file that is gone is a waiver nobody can retire. Prune them in stage 6; six real entries remain.
