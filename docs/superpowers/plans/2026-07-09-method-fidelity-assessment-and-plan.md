...
- **C1. `composegen` — ✅ DONE 2026-07-10** (appgen step 8): `cmd/server/main.gen.go`/`config.gen.go`
  live, `run()` deleted, hand seam = `hooks.go`; dogfood conversion complete.
- **C2. `transportgen` + deployment model — ✅ DONE 2026-07-10** (appgen steps 5, 7, incl. configgen);
  delivered-app packaging (step 10) remains per the spec's earmarks.
