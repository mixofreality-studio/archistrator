...
1. **Temporal — DECIDED (founder):** decided not a volatility. Temporal is a operation concept decision & implementation detail.
 Strike "Durable Execution Runtime" from slot 3; record the commitment in operational concepts (H3's first amendment).
...
**Status refresh (2026-07-10, verified against main @ v0.8.41 after appgen steps 1–9 landed):**

- **Landed since this plan was written:** the full appgen pipeline — temporalgen 5-manager migration,
  typed OAS, transportgen SDK, deployment model + configgen, and **composegen** (`cmd/server/main.gen.go` +
  `config.gen.go` live, `run()` deleted, hand seam = `hooks.go`). **Fidelity C1 and C2 are DONE.** The I3
  aside about the seam-adapter pattern now reads historically — the composition root is generated.
- **O5 narrowed:** the activity→component join is no longer title-substring-only — explicit `contractKey`
  preferred-exact-match with heuristic fallback (`CN/eligibility.go:92-116`, commit a754b6c). The
  **silent skip on no-match remains** (`:79`) — that half of O5 still needs the surfaced-finding fix.
- **O4 narrowed:** the pump now carries a pause gate (`CN/workflow.go:341` — ReceiveAsync before every
  dispatch, quiet-without-ContinueAsNew), but `PauseProject` still signals only `{proj}:construction`
  (`pauseTargetWorkflowID`, `CN/constructionmanager.go:394`) and nothing routes the signal to the pump's
  `{proj}:nextActivity:{tick}` id — the gate exists, unfed. The remaining fix is routing only.
- **Re-verified unchanged:** O1 (declaration-order eligibility), O2/O3 (phase leaks, split seal paths),
  O8; the three severe §6.3 prompt gaps (priors are now delivered as `writePriorsPointer` slot-name lists
  the agent reads via `getCommittedSlot` — the *mechanism* improved, but scrub still gets Mission+Glossary
  only, standardCheck still System+OperationalConcepts only, Phase-2 network still never names the System
  slot); coreUseCases still omits ScrubbedRequirements. The agent-surface plan (2026-07-06) and the
  seam-adapter cleanup (2026-07-09) are both **still unexecuted** — seam mirrors verified present in all
  three managers, revenue zombie spine present, `Settlement*` naming still in the intervention contract.
