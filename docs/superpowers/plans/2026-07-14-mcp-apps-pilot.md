...
- [ ] **Step 2:** Record earmarks where they're tracked: platform eslint-config-web needs the same DAG change released (vendored copy edited here) PLUS an `element.innerHTML` assignment ban (final-review addition); UserContext fetch-ban exemption removal (relocate session probe: src/api helper → src/hooks hook → container-mounted provider; see eslint.platform.config.js burn-down comment); production OAuth on `/mcp` is a release-blocker before real state meets a connector; screen-migration checklist (remaining screens × {purify, container, registry, annotation, inline-compact variant}) — VolatilityMap + OperationalConceptsView first, they error in the iframe today (T11); `visibility:["app"]` unused for now; loud prod-mode failure when ARCHISTRATOR_WEBAPP_ORIGIN is unset (silent localhost default is a footgun); platform OpUI docstring must note the view id also drives the shell registry; SystemDesignViewProps pending/staleAck grouping before the next screen copies the shape; testing tiers per spec §6b (transport-parameterized systemtests + instrumented-harness mcptests) BEFORE the migration wave; dev-mode stub variant so `npm run dev` can serve the shell (vite preview is required today).
