...
| `recordActivityStarted` | written in **both shipping profiles**: cloud (GitHub: a PR rail + repo resolve, real installation token) and local (GitLocal: status records fire with a zero credential). `gitOn` is just `wf.GitStatus != nil` (`constructactivity.go` `startedCred`), and only unit-test setups leave `GitStatus` nil. *Corrected 2026-09-12 (architect ruling): this row used to say "written only when `gitOn` (the git profile)", which read as "not in local" and was the false premise of Q1.* Stored `StartedAt`/`CompletedAt` already ride the get-project wire but `mapConstructionRow` drops them |
...
| DC2 | Probe the existing per-activity session route only for rows the pump **started and has not finished** (`startedAt` set, no `completedAt`, not `failed`). Zero probes today; ≤ the supervision cap in practice. | Fix A deleted 29 per-load probes; this probes only where a workflow can exist. Q1 (ruled: keep it; per-child probes also find orphans). |
...
| DC11 | The shared toolbar's search/kind/layer/scope apply to Tasks through the same `applyToolbarToActivities` pipeline (an owed row shows iff its activity passes). The owed set — and so the TASKS badge, computed in ONE place — reads the **evidence view** like every other surface; "Observed only" cannot change the set (a gate is the live session, a failure is the pump's own `failureReason`, both kept by the view), pinned by a test. What the view may strip is a mixed-ledger row's current phase, which then reads "unreported". (Main's Task 8 is moving the old `in-review` badge onto the evidence view too — a small, expected merge touch on the same lines.) | §7.4: toolbar state persists across lenses; orchestrator input 2026-09-12. |
...
**Q5 — Architecture + product: the §9.8 throughput metric (time from an agent stopping at a gate to its resume, and the human wall-clock share of it) is unmet, and no surface can measure it honestly today.** The client never learns when a gate OPENED, so it can time only "decision sent → resume observed", the machine's share after the click; it cannot see the human's share, which is the number the PM defined the wave by. Nothing server-side records either time. *Recommendation:* capture it where it happens, in the workflow. (1) Now: `awaitPhaseDecision` and the merge hold stamp `workflow.Now` on entering the stage and on the decision signal, expose the open time as `awaitingSince` (Q2), and emit one structured `construction.gate.decided` log line/metric with `{activityId, gateKey, round, waitSeconds, decision}`. That meets "even if only instrumented", and the TASKS lens then renders WAITING and the stall header from the same field. (2) Durable, once the RecordTaskAttempt writer lands (Stage A earmark): the gate task's attempt carries `startedAt` = gate opened and `endedAt` = decision, `actor: human`, so the metric becomes Σ(endedAt − startedAt) over human-actor gate attempts, computed on read and joinable by attempt id. Both touch `awaitPhaseDecision`, so this waits on Q2's ruling. Nothing is built on this branch.
...
- Probe fan-out is per page load while in flight (≤ the supervision cap). Q1 ruled to keep it.
