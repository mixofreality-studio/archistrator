...
- **D3 — The alarm follows App C, including its carve-out.** R5 says any upward or sideways edge alarms; it also says the view's purpose is "a live App C layering check". App C §3.4 (the-method-layers) forbids sideways calls "except queued Manager → Manager". (Correction, follow-up round 1: the server's Design-Health rule at `designhealthengine.go:2522` is LOOSER — it exempts any queued same-layer call. The architect's Q1 ruling keeps the graph on App C's stricter rule — both endpoints Managers, `mode = queued`, same row — and earmarks the server rule; see `arch-graph-q1q2-ruling.md` and spec R5.) The two live sideways edges are that sanctioned case. Alarming them would render a design defect App C says does not exist, so: `up` → alarm; `sideways` → alarm unless `queued` and Manager→Manager (drawn dashed, as every queued call already is, and counted separately). Q1 asks the architect to ratify.
...
- **D8 — The ribbon is HTML above the canvas.** Milestones never pan away. Each shows id, name and its fan-in `n feeders`; the completion count `k/n` (complete = `percentComplete === 100`, the App A binary exit arithmetic) is itself an AGGREGATE, so per §9.2 it renders only when every feeder's evidence is `observed` and reads **"—"** otherwise — never a badged number — with the hatch and the spelled-out `≈ RECONSTRUCTED` stamp when any feeder is reconstructed. `M0`, having no fan-in, shows `gates N` instead of a state. No event times (D2). Hovering a milestone hover-focuses its feeders (or, for M0, what it gates). *(Amended during Task 3 on the coordinator's §9.2 review note.)*
- **D12 — No aggregate widget is reused.** `HeadStateRollup`, `EvTrackingChart` and `NearCriticalFloat` are NOT mounted: none carries a `worstOrigin` gate (mounting one would launder reconstructed work into a total, §9.2), and the EV curve counts reconstructed work against a stale `TotalWeeks` that the founder is ruling on. The only numbers the lens shows are structural — layering alarms, hollow components, milestone fan-in — plus the §9.2-gated `k/n` above.
...
**Interfaces — produces:** `gateRibbonFor(milestones, dependencies, nodes): RibbonMilestone[]` where `RibbonMilestone = { id; name; feeders: string[]; gates: string[]; complete?: number; provenance: ProvenanceOrigin }` — `complete` is ABSENT unless every feeder's evidence is `observed` (§9.2, D8).

- [ ] **Step 1: Failing tests** — milestones keep network order; `M1` feeders are its `dependsOn`; with every feeder observed, `complete` counts those at `percentComplete === 100`; **with any feeder reconstructed or unrecorded, `complete` is absent** (no laundered aggregate); `M0` (no `dependsOn`) has zero feeders and `gates` = every activity that lists it; provenance is the worst over feeders (reconstructed feeder → reconstructed; none → `unknown`); a feeder id absent from the tree counts as not complete and not observed (never crashes).
