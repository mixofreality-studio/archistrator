...
**Architecture (amended, founder-acknowledged — the delivered shape is a two-binary managed-child model, not the single in-process HTTP listener originally sketched here):** Driver ≠ worker, and **Claude Code is the process manager** (Serena pattern): `.mcp.json` registers `archistrator mcp` as a **stdio** MCP server, so starting Claude Code in the repo auto-spawns the whole stack. `archistrator mcp` (`cmd/archistrator`, the thin CLI entry point) does NOT boot the HTTP listener itself — Go refuses to import a second `package main` (cmd/server is one), so the CLI instead locates and supervises `archistrator-server` (`cmd/server`, the SAME composition root the cloud image runs) as a **child process bound to loopback**, then bridges the stdio MCP transport it speaks to Claude Code into that child's own `/mcp` HTTP mount (`cmd/archistrator/serverchild.go`, `mcpserve.go`). The spawned child boots the embedded SPA + REST API + `/mcp` + embedded Temporal, all local-profile-configured (loopback listen address, git-local project state, dev-mode auth gated to loopback). `scripts/build-local.sh` produces the three binaries this model needs side by side at the repo root: `archistrator` (the CLI/stdio-bridge), `archistrator-server` (the composition root, `-tags localdist` with the embedded SPA staged in), and `aiarch-state-mcp` (the construct-verb rig the local construction executor attaches to headless `claude` via `--mcp-config`) — `locateServerBinary`/`locateStateMCPBinary` each discover their target as a sibling of the currently-running executable. All LLM work (drafting *and* construction) runs as headless `claude -p` subprocesses riding the user's Pro/Max OAuth — verified: headless mode uses subscription auth, no API key needed. The SPA (embedded in `archistrator-server`, served at `/`) is the render surface; its URL is surfaced through MCP tool responses so the driver can hand it to the user. `.aiarch/` shape stays byte-identical to cloud — the repo itself is the migration artifact to hosted mode. MCP Apps is explicitly NOT a surface here (Claude Code doesn't render `ui://`; claude.ai/Desktop rendering has an open bug — ext-apps#671).
