...
- Lint gate (`make lint`, runs WITHOUT `GOWORK=off`): golangci-lint with `gochecksumtype`, `gocognit`, `gosec`, plus `make encapsulation-check` (`TestGeneratedOnlyPublic` — do not add new exported symbols to `projectstate` unless intended to be public; `Profile`/`ProfileFor`/`DeriveType`/`DeriveVariant` ARE intended public), `make sumtype-check`. If you add a `switch` over the sealed `ActivityType`, keep it exhaustive.
...
- [ ] **Step 4: Full server suite + vet + lint**
...
```
Expected: all green. `encapsulation-check` (`TestGeneratedOnlyPublic`) must still pass with the new exported `Profile`/`ProfilePhase`/`ProfileFor`/`DeriveType`/`DeriveVariant`; if it flags them, they are legitimately public API — follow the guidance the test prints for registering intended-public symbols (do NOT unexport them; the pump in another package consumes `ProfileFor`/`PhaseIDs`).
