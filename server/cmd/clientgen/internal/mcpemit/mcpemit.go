...
func genTools(doc *projectmodel.Doc, enums map[string]enumDef, opts Options) ([]byte, error) {
	...
	enumNames := usedEnumNames(doc, enums)
	...
	if err := emitHandlerAndRegister(&b, doc, opts, mgrPrefix, iface); err != nil {
		return nil, err
	}

	emitInputOutputTypes(&b, doc)
...
// usedEnumNames returns the sorted set of enum $def names referenced by any op
// param — the enums for which a schema helper is emitted.
func usedEnumNames(doc *projectmodel.Doc, enums map[string]enumDef) []string {
	used := map[string]bool{}
	for _, op := range doc.Interface.Operations {
		for _, p := range op.Params {
			if _, ok := enums[p.Schema.RefName()]; ok {
				used[p.Schema.RefName()] = true
			}
		}
	}
...
// emitHandlerAndRegister writes the shellResourceURI const (iff any op is
// view-bearing), the Handler struct, and the Register method that adds every op
// as an mcp.Tool.
func emitHandlerAndRegister(b *strings.Builder, doc *projectmodel.Doc, opts Options, mgrPrefix, iface string) error {
	...
	for _, op := range doc.Interface.Operations {
		...
		if strings.TrimSpace(desc) == "" {
			return fmt.Errorf("mcpemit: no documentation for operation %q on %s (add it to clientgen's op-doc table)", op.Name, iface)
		}
...
// emitInputOutputTypes writes the per-op Input/Output struct types.
func emitInputOutputTypes(b *strings.Builder, doc *projectmodel.Doc) {
	for _, op := range doc.Interface.Operations {
...
func writeHelpers(b *strings.Builder) {
	...
	b.WriteString("// fixUUIDStrings walks an inferred schema and retypes every uuid.UUID ([16]byte)\n")
	b.WriteString("// property from the SDK's structural inference (an array of 16 0-255 bytes) to\n")
	b.WriteString("// the schema the value actually marshals as on the wire: a string in RFC 4122\n")
	b.WriteString("// form. It MUST run before relaxRawJSON, which would otherwise blank the same\n")
	b.WriteString("// node as an indistinguishable raw-byte-array signature (T11 finding).\n")
	b.WriteString("func fixUUIDStrings(s *jsonschema.Schema) {\n")
	b.WriteString("\tif s == nil {\n\t\treturn\n\t}\n")
	b.WriteString("\tif isUUIDArray(s) {\n\t\t*s = jsonschema.Schema{Type: \"string\", Format: \"uuid\"}\n\t\treturn\n\t}\n")
	b.WriteString("\tfor _, p := range s.Properties {\n\t\tfixUUIDStrings(p)\n\t}\n")
	b.WriteString("\tfixUUIDStrings(s.Items)\n")
	b.WriteString("\tfixUUIDStrings(s.AdditionalProperties)\n")
	b.WriteString("\tfor _, p := range s.PrefixItems {\n\t\tfixUUIDStrings(p)\n\t}\n")
	b.WriteString("}\n\n")

	b.WriteString("// isUUIDArray reports whether a schema is the jsonschema-go signature of a Go\n")
	b.WriteString("// uuid.UUID ([16]byte): the same raw-byte-array shape as isRawByteArray, further\n")
	b.WriteString("// narrowed to exactly 16 items (a fixed-size array), which is unique to UUIDs\n")
	b.WriteString("// among this codebase's byte-carrier fields.\n")
	b.WriteString("func isUUIDArray(s *jsonschema.Schema) bool {\n")
	b.WriteString("\tif !isRawByteArray(s) {\n\t\treturn false\n\t}\n")
	b.WriteString("\treturn s.MinItems != nil && *s.MinItems == 16 && s.MaxItems != nil && *s.MaxItems == 16\n")
	b.WriteString("}\n\n")
	...
	b.WriteString("// of the inferred shape is preserved. The permissive schema carries a\n")
	b.WriteString("// Description so it marshals as a JSON OBJECT (`{\"description\":...}`) rather\n")
	b.WriteString("// than the zero-value Schema{}, which the library marshals as bare JSON `true`\n")
	b.WriteString("// — valid JSON Schema, but rejected by the TS MCP SDK's Zod validator, which\n")
	b.WriteString("// requires every property schema to be an object (T11 finding).\n")
