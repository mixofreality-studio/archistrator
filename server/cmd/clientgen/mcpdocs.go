...
var mcpOpDocs = map[string]map[string]string{
	...
	"ConstructionManager": {
		...
		"QueryActivityView":       "Return one construction activity's whole lifecycle in one read: its task DAG (every task, its dependencies, its state) grouped into the lifecycle phases and their earned-value weights, each task's revision history with the episode, send-back note and anchored comments behind it, and the reviewer set at the gate it is waiting at. Read-only.",
