...
func TestServe_BootsStack_AnswersMCPOverHTTP_And_SIGTERMLeavesNoOrphans(t *testing.T) {
	...
	before := childPIDs(cmd.Process.Pid)
	if len(before) == 0 {
	...
	// MCP initialize over HTTP — the ONLY MCP surface now (no stdio bridge).
	...
	// deliberately does NOT assert "/" returns 200: buildBinaries above
	// builds archistrator-server WITHOUT -tags localdist (no embedded webApp
	// dist to stage), so mountSPA's documented no-op arm correctly 404s here
	// (see cmd/server/spa_handler_test.go's TestMountSPANoOpWhenBuiltWithoutLocaldistTag)
	// — SPA-serving itself is Task 4's own gate, exercised for real by
	// scripts/build-local.sh's -tags localdist build in this task's live
	// verification, not by this unit-scoped process test.
	...
	// Zero orphans: every child pid observed above must be gone within a
	// bounded window.
