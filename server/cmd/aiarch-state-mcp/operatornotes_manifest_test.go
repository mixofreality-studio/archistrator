...
// narrows every bound session's tool surface, so getOperatorNotes reaches a
...
// grants getOperatorNotes, and a session bound to it registers the tool.
func TestOperatorNote_EveryConstructStepKeepsTheTool(t *testing.T) {
	...
	for command, m := range methodassets.Manifests() {
		...
		if !registeredMCPToolNames(t, s)[getOperatorNotesTool] {
			t.Errorf("%s: a construct step must keep getOperatorNotes (the method-assets manifest grant)", command)
		}
	}
	if construct == 0 {
...
// dispatch rails stamp it) still registers getOperatorNotes and serves the note.
func TestRig_OperatorNoteOverStdio_BoundToAConstructStep(t *testing.T) {
	...
	if !names[getOperatorNotesTool] {
		t.Fatalf("service-construction (manifest-bound) must register getOperatorNotes: %v", names)
	}
	res := callTool(ctx, t, session, getOperatorNotesTool, map[string]any{})
	if res.IsError || !strings.Contains(contentText(res), "\n"+note+"\n") {
		t.Fatalf("getOperatorNotes did not return the note verbatim: %s", contentText(res))
	}
}
