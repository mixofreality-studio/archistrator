...
func run(logger *slog.Logger) error {
	...
	// Composition-root logging SEAM over the four web/MCP-exposed Managers (managerlog.go).
	// Each wrapper is handed to BOTH the REST Handler and the MCP tool Handler below, so
	// every Infrastructure-kind error surfaced to a client — through either transport — is
	// logged once server-side with the operation name, project id, and the complete cause
	// chain (the transport body stays opaque). The UNWRAPPED Managers stay in scope for the
	// RegisterWorker calls above (which type-assert back to the concrete impls).
	loggedSystemDesign := loggingSystemDesignManager{inner: manager, log: logger}
	loggedProjectDesign := loggingProjectDesignManager{inner: projectDesignManager, log: logger}
	loggedConstruction := loggingConstructionManager{inner: constructionManager, log: logger}
	loggedOperations := loggingOperationsManager{inner: operationsManager, log: logger}
	...
	genServer := web.NewServer(
		cfg.Dev,
		tokenValidator,
		&systemdesignweb.Handler{Manager: loggedSystemDesign, Security: sec},
		&projectdesignweb.Handler{Manager: loggedProjectDesign, Security: sec},
		&constructionweb.Handler{Manager: loggedConstruction, Security: sec},
		&operationsweb.Handler{Manager: loggedOperations, Security: sec},
	)
	...
	root.Handle("/mcp", newMCPHandler(cfg.Dev, tokenValidator, loggedSystemDesign, loggedProjectDesign, loggedConstruction, loggedOperations))
