...
func mustFold(t *testing.T, project, schema []byte, key, dir string) []byte {
	t.Helper()
	out, err := Fold(project, schema, key, dir)
	if err != nil {
		t.Fatalf("Fold: %v", err)
	}
	return out
...
func TestFold_ReplacesOnlyTitleDefsInterface(t *testing.T) {
	...
	out := mustFold(t, []byte(fixtureProject), schema, "artifactAccess", "internal/resourceaccess/artifact")
...
func TestFold_Idempotent(t *testing.T) {
	schema := schemaFor("artifact contract v2", "ArtifactRef", "ArtifactAccess", "resourceaccess")
	once := mustFold(t, []byte(fixtureProject), schema, "artifactAccess", "internal/resourceaccess/artifact")
	twice := mustFold(t, once, schema, "artifactAccess", "internal/resourceaccess/artifact")
...
func TestFold_GoPackageMismatchRefused(t *testing.T) {
	schema := schemaFor("artifact contract v2", "ArtifactRef", "ArtifactAccess", "resourceaccess")
	_, err := Fold([]byte(fixtureProject), schema, "artifactAccess", "internal/resourceaccess/somewhereelse")
	if err == nil {
...
func TestFold_CreatesNewEntry(t *testing.T) {
	...
	out := mustFold(t, []byte(fixtureProject), schema, "billingEngine", "internal/engine/billing")
	...
	twice := mustFold(t, out, schema, "billingEngine", "internal/engine/billing")
...
func TestFold_UnknownLayerOnCreateRefused(t *testing.T) {
	...
	_, err := Fold([]byte(fixtureProject), schema, "mysteryThing", "internal/mystery/thing")
	if err == nil {
...
func TestFold_RejectsMissingSchemaFields(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := Fold([]byte(fixtureProject), []byte(tc.schema), "artifactAccess", "internal/resourceaccess/artifact")
			if err == nil {
...
`
	...
	if _, err := Fold([]byte(dup), schema, "a", "internal/engine/x"); err != nil {
...
func TestFold_RequiresKeyAndDir(t *testing.T) {
	...
	if _, err := Fold([]byte(fixtureProject), schema, "", "internal/x"); err == nil {
		t.Fatal("expected error for empty key")
	}
	if _, err := Fold([]byte(fixtureProject), schema, "artifactAccess", ""); err == nil {
...
func TestFold_MissingServiceContracts(t *testing.T) {
	schema := schemaFor("t", "X", "X", "engine")
	_, err := Fold([]byte(`{"id":"x"}`), schema, "artifactAccess", "internal/resourceaccess/artifact")
	if err == nil {
