...
// it's being redrafted", and the PM-critique-unresolved warning is one), so the
// minimal types are RELOCATED here — owned by the Manager that surfaces them — rather
// than keeping a whole dead Engine for a value type.
//
...
const (
	...
	// SeverityWarning is advisory-only (e.g. an unresolved PM critique surfaced at the gate).
	SeverityWarning
...
func (s Severity) MarshalJSON() ([]byte, error) {
	...
	if !ok {
		return nil, fmt.Errorf("systemdesign: Severity(%d) has no wire name", int(s))
	}
...
func (s *Severity) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(data, &name); err == nil {
		...
		if !ok {
			return fmt.Errorf("systemdesign: %q is not a recognized Severity wire name", name)
		}
		*s = v
		return nil
	}
	...
	if err := json.Unmarshal(data, &ordinal); err != nil {
		return fmt.Errorf("systemdesign: Severity must be a string wire name or integer ordinal: %w", err)
	}
	*s = Severity(ordinal)
	return nil
}
...
type Location struct {
	...
	Section string `json:"section"` // human-readable locus, e.g. "core use case 3"
}
