...
// collapses the retry) — systemDesignManager.md §6.4; D-PA §3.
//
...
// restart, executes under the same deterministic workflow ID gtdapp:<kind>). Without
// the RunID the dispatch token "${workflowId}:${activityId}" of a fresh session
// COLLIDES with a predecessor session's token: the constructionpipeline RA's
// run-name dedup (aiarch-cp-<sha256(key)>) would then converge on the predecessor's
// already-completed GitHub run, so no new run is dispatched, observe sees the stale
// success, and OpenPullRequest 422s on the zero-new-commit branch. The RunID is a
// property of the workflow EXECUTION (fixed for the run's lifetime, identical across
// activity retries, unchanged by replay) — it is replay-stable, unlike wall-clock.
// It scopes EVERY per-session write key (not just the dispatch) to its session, so a
// new run's Commit/Stage/Reject never dedups against a predecessor run's ledger entry
// either. Same key content still hashes to a valid 32-hex aiarch-cp-<token> run name,
// so the RA run-name + concurrency-group contract is unchanged.
...
func (wf *workflows) RejectArtifactActivity(ctx context.Context, a mutateArtifactArgs) (projectstate.Version, error) {
	...
	//
	// REVIEW LEDGER (review-ledger §2): when the substrate supports the durable ledger,
	// record the Rejected status flip AND append the reviewer's comments to the slot's
	// ReviewThread in ONE atomic commit (crash-safe, idempotent on the deterministic ids).
	// branch=="" (dormant rail) still appends — the ledger verb handles the main path. Only a
	// substrate WITHOUT the ledger extension falls back to the plain (comment-dropping) reject.
