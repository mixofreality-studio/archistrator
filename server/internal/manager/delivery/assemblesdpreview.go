...
func (wf *pdWorkflows) AssembleSDPReviewWorkflow(ctx workflow.Context, in sdpReviewInput) error {
	state := &pdCoAuthorState{
		projectID:    in.ProjectID,
		artifactKind: KindSdpReview,
		stage:        ProjectStageAssemblingSDP,
	}
	...
	for attempt := 0; ; attempt++ {
		state.stage = ProjectStageAssemblingSDP
		...
		workflow.GetSignalChannel(ctx, pdSignalSDPDecision).Receive(ctx, &sig)
...
func (wf *pdWorkflows) stageReview(ctx workflow.Context, projectID ProjectID, review *projectstate.SdpReview, headVersion *projectstate.Version) error {
...
func (wf *pdWorkflows) commitReview(ctx workflow.Context, projectID ProjectID, headVersion *projectstate.Version, approver string) error {
	v, err := wf.applyRecovering(ctx, projectID, "", *headVersion, func(expected projectstate.Version) (projectstate.Version, error) {
...
// path.
func (wf *pdWorkflows) rejectReview(ctx workflow.Context, projectID ProjectID, notes string, headVersion *projectstate.Version) error {
	v, err := wf.applyRecovering(ctx, projectID, "", *headVersion, func(expected projectstate.Version) (projectstate.Version, error) {
