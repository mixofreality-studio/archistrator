...
// ActivityExecutionReadActivityExecution wraps activityExecutionAccess.readActivityExecution.
// Registered as "activityExecutionAccess.readActivityExecution".
func (a *genActivities) ActivityExecutionReadActivityExecution(ctx context.Context, projectID projectstate.ProjectID, activityID string) (projectstate.ActivityExecution, error) {
	v, err := a.ActivityExecution.ReadActivityExecution(fwra.Context{Context: ctx, IdempotencyKey: genActivityIdempotencyKey(ctx)}, projectID, activityID)
	return v, fwmanager.MapError(err)
}
