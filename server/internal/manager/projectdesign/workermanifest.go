...
// the BranchAware/Ledger/Provenance capability type-assertions, the review-ledger branch
// mutations, and the free-function managed-scaffold sync — activities_custom.go /
// reviewledger.go / gitrail.go), the per-activity option-preset hook, and the genActivities
// dep threading. It also hosts the external RegisterManagerWorker entrypoint the composition
...
// and the published Rail (the free-function SyncManagedScaffold + the gitEnabled gate). The
// migration routes the contract-backed RA ops (readProjectVersion / advancePhase / submit /
// observe / the six rail verbs) through the generated invoker surface.
func (m *projectDesignManager) WorkerManifest() genWorkerManifest {
	...
	return genWorkerManifest{
		...
		// the CustomActivities manifest surface). The methods stay — the projectdesign
		// rewire task (B9) migrates their call sites onto the generated DesignSession
