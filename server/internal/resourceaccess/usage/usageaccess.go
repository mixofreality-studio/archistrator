...
//
// The one unified log meters BOTH construction-token consumption (build phase
// — units_unit 'construction-token', no operated app) AND hosting/compute
// consumption (operation phase — 'compute-unit-second' / 'storage-byte-month'
...
// Construction-token facts are build-phase and have NO operated app — the
// zero uuid here means "absent" and persists as NULL. Hosting facts carry a
// non-zero id.
