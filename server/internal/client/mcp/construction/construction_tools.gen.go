// Code generated by framework-go-mcp-generator. DO NOT EDIT.
// Source contract:

package construction

import (
	"context"
	"errors"
	"fmt"

	"github.com/modelcontextprotocol/go-sdk/mcp"

	fwmanager "github.com/mixofreality-studio/archistrator-platform/framework-go/manager"
	"github.com/mixofreality-studio/archistrator-platform/framework-go/utilities/security"
	mgr "github.com/mixofreality-studio/archistrator/server/internal/manager/construction"
)

// Handler binds the ConstructionManager manager to MCP tools.
type Handler struct {
	Manager mgr.ConstructionManager
}

// Register adds every operation as an mcp.Tool on srv. The SDK infers each
// tool's input/output JSON Schema from the typed handler signatures.
func (h *Handler) Register(srv *mcp.Server) {
	mcp.AddTool(srv, &mcp.Tool{Name: "constructionExecuteNextActivity", Description: "ExecuteNextActivity on the Construction manager."}, h.handleExecuteNextActivity)
	mcp.AddTool(srv, &mcp.Tool{Name: "constructionGetSessionState", Description: "GetSessionState on the Construction manager."}, h.handleGetSessionState)
	mcp.AddTool(srv, &mcp.Tool{Name: "constructionOverrideActivity", Description: "OverrideActivity on the Construction manager."}, h.handleOverrideActivity)
	mcp.AddTool(srv, &mcp.Tool{Name: "constructionPauseProject", Description: "PauseProject on the Construction manager."}, h.handlePauseProject)
	mcp.AddTool(srv, &mcp.Tool{Name: "constructionRunReplanSweep", Description: "RunReplanSweep on the Construction manager."}, h.handleRunReplanSweep)
}

type executeNextActivityInput struct {
	ProjectID mgr.ProjectID `json:"projectID"`
	TickID    string        `json:"tickID"`
}

type executeNextActivityOutput struct {
	Result mgr.PumpResult `json:"result"`
}

type getSessionStateInput struct {
	ProjectID  mgr.ProjectID   `json:"projectID"`
	ActivityID *mgr.ActivityID `json:"activityID"`
}

type getSessionStateOutput struct {
	Result mgr.ConstructionSessionView `json:"result"`
}

type overrideActivityInput struct {
	ProjectID  mgr.ProjectID        `json:"projectID"`
	ActivityID mgr.ActivityID       `json:"activityID"`
	Override   mgr.ActivityOverride `json:"override"`
}

type overrideActivityOutput struct{}

type pauseProjectInput struct {
	ProjectID mgr.ProjectID `json:"projectID"`
	Reason    string        `json:"reason"`
}

type pauseProjectOutput struct{}

type runReplanSweepInput struct {
	ProjectID *mgr.ProjectID `json:"projectID"`
	TickID    string         `json:"tickID"`
}

type runReplanSweepOutput struct {
	Result mgr.ReplanSweepResult `json:"result"`
}

// handleExecuteNextActivity is the MCP tool handler for the ExecuteNextActivity operation.
func (h *Handler) handleExecuteNextActivity(ctx context.Context, _ *mcp.CallToolRequest, in executeNextActivityInput) (*mcp.CallToolResult, executeNextActivityOutput, error) {
	var out executeNextActivityOutput
	principal, _ := security.PrincipalFrom(ctx)
	rc := fwmanager.Context{Context: ctx, Principal: principal}
	result, err := h.Manager.ExecuteNextActivity(rc, in.ProjectID, in.TickID)
	if err != nil {
		return nil, out, mapManagerError(err)
	}
	out.Result = result
	return nil, out, nil
}

// handleGetSessionState is the MCP tool handler for the GetSessionState operation.
func (h *Handler) handleGetSessionState(ctx context.Context, _ *mcp.CallToolRequest, in getSessionStateInput) (*mcp.CallToolResult, getSessionStateOutput, error) {
	var out getSessionStateOutput
	principal, _ := security.PrincipalFrom(ctx)
	rc := fwmanager.Context{Context: ctx, Principal: principal}
	result, err := h.Manager.GetSessionState(rc, in.ProjectID, in.ActivityID)
	if err != nil {
		return nil, out, mapManagerError(err)
	}
	out.Result = result
	return nil, out, nil
}

// handleOverrideActivity is the MCP tool handler for the OverrideActivity operation.
func (h *Handler) handleOverrideActivity(ctx context.Context, _ *mcp.CallToolRequest, in overrideActivityInput) (*mcp.CallToolResult, overrideActivityOutput, error) {
	var out overrideActivityOutput
	principal, _ := security.PrincipalFrom(ctx)
	rc := fwmanager.Context{Context: ctx, Principal: principal}
	if err := h.Manager.OverrideActivity(rc, in.ProjectID, in.ActivityID, in.Override); err != nil {
		return nil, out, mapManagerError(err)
	}
	return nil, out, nil
}

// handlePauseProject is the MCP tool handler for the PauseProject operation.
func (h *Handler) handlePauseProject(ctx context.Context, _ *mcp.CallToolRequest, in pauseProjectInput) (*mcp.CallToolResult, pauseProjectOutput, error) {
	var out pauseProjectOutput
	principal, _ := security.PrincipalFrom(ctx)
	rc := fwmanager.Context{Context: ctx, Principal: principal}
	if err := h.Manager.PauseProject(rc, in.ProjectID, in.Reason); err != nil {
		return nil, out, mapManagerError(err)
	}
	return nil, out, nil
}

// handleRunReplanSweep is the MCP tool handler for the RunReplanSweep operation.
func (h *Handler) handleRunReplanSweep(ctx context.Context, _ *mcp.CallToolRequest, in runReplanSweepInput) (*mcp.CallToolResult, runReplanSweepOutput, error) {
	var out runReplanSweepOutput
	principal, _ := security.PrincipalFrom(ctx)
	rc := fwmanager.Context{Context: ctx, Principal: principal}
	result, err := h.Manager.RunReplanSweep(rc, in.ProjectID, in.TickID)
	if err != nil {
		return nil, out, mapManagerError(err)
	}
	out.Result = result
	return nil, out, nil
}

// --- error mapping ---------------------------------------------------------

func mapManagerError(err error) error {
	var me *fwmanager.Error
	if errors.As(err, &me) {
		return fmt.Errorf("%s: %s", me.Kind.String(), me.Detail)
	}
	return err
}
